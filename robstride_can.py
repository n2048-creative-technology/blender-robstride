...
class RobStrideManager:
    ...
    def scan(self) -> List[Dict[str, Any]]:
        # Prefer real network if connected; fall back to simulation only when offline
        if self.connected:
            ...
            # Connected but no backend returned results
            return []
        ...
        # Offline and not simulating
        return []

    def set_pid(self, node_id: int, kp: float, ki: float, kd: float) -> None:
        # Prefer real network if connected; simulation is a no-op
        if self.connected:
            if robstride_lib is not None:
                try:
    ...
    def enable_node(self, node_id: int, enable: bool) -> None:
        # Prefer real network if connected; simulation ignores enable/disable
        if self.connected:
            if robstride_lib is not None:
                try:
    ...
    def send_position(self, node_id: int, value: float) -> None:
        # Prefer real network if connected; simulation stores last value
        if self.connected:
            if robstride_lib is not None:
                try:
    ...
    def read_position(self, node_id: int) -> float:
        # Prefer real network if connected; simulation synthesizes position
        if self.connected:
            if robstride_lib is not None:
                try:
    ...
    def node_status(self, node_id: int) -> bool:
        # Prefer real network status if connected; otherwise treat sim nodes as online
        if self.connected:
            if self._co_net is not None and canopen is not None:
